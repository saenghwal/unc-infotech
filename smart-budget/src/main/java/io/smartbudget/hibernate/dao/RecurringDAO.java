...
public class RecurringDAO extends AbstractDAO<Recurring> {
    ...
    public List<Recurring> findRecurrings(User user) {
        return currentSession().createQuery("SELECT r FROM Recurring r JOIN r.budgetType budgetType WHERE budgetType IN (SELECT smartbudget.budgetType FROM Budget budget WHERE smartbudget.user = :user)")
    ...
    public Recurring find(User user, long recurringId) {
        return (Recurring)currentSession().createQuery("SELECT r FROM Recurring r JOIN r.budgetType budgetType WHERE budgetType IN (FROM Budget budget WHERE smartbudget.user = :user and r.id = :id)")
                .setParameter("user", user)
